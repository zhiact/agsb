...
# 全局变量
INSTALL_DIR = Path.home() / ".frpsb"  # 用户主目录下的隐藏文件夹，避免root权限
CONFIG_FILE = INSTALL_DIR / "config.json"
SB_PID_FILE = INSTALL_DIR / "sbpid.log"
FRPC_PID_FILE = INSTALL_DIR / "frpcpid.log"
LIST_FILE = INSTALL_DIR / "list.txt"
LOG_FILE = INSTALL_DIR / "argo.log"
DEBUG_LOG = INSTALL_DIR / "python_debug.log"
FRPS_ADDR="129.153.93.199"
FRPS_PORT="443"
FRPS_TOKEN="6FO258BxZYD6w9LM"
FRPS_REMOTE_PORT="60000"
...
def print_info():
    ...
    print("\033[36m│                \033[33m✨ frpSB Python3 一键脚本 ✨               \033[36m│\033[0m")
    print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
    print("\033[36m│ \033[32m作者: zhiact                                                  \033[36m│\033[0m")
    print("\033[36m│ \033[32mGithub: https://github.com/zhiact/                    \033[36m│\033[0m")
    print("\033[36m│ \033[32mYouTube:        \033[36m│\033[0m")
    print("\033[36m│ \033[32mTelegram:                    \033[36m│\033[0m")
...
def print_usage():
    ...
    print("  \033[36mpython3 frpsb.py\033[0m              - 安装并启动服务")
    print("  \033[36mpython3 frpsb.py install\033[0m      - 安装服务")
    print("  \033[36mpython3 frpsb.py status\033[0m       - 查看服务状态和节点信息")
    print("  \033[36mpython3 frpsb.py cat\033[0m          - 查看单行节点列表")
    print("  \033[36mpython3 frpsb.py update\033[0m       - 更新脚本")
    print("  \033[36mpython3 frpsb.py del\033[0m          - 卸载服务")
    print()
...
# 生成链接
def generate_links(uuid_str):
    write_debug_log(f"生成链接:  uuid_str={uuid_str}")
    ...
    # 443端口 - 104.16.0.0
    config1 = {
        "ps": f"FRP-{hostname}-443",
        "add": f"{FRPS_ADDR}",
        "port": f"{FRPS_REMOTE_PORT}",
        "id": uuid_str,
        "aid": "0",
        "net": "ws",
        "type": "none",
    ...
    with open(str(LIST_FILE), 'w') as f:
        ...
        f.write("\033[36m│                    \033[33m✨ frpSB 节点信息 ✨                   \033[36m│\033[0m\n")
        f.write("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m\n")
        ...
        f.write("\033[36m│ \033[32m查看节点信息: \033[0mpython3 frpsb.py status\n")
        f.write("\033[36m│ \033[32m查看所有节点(一行一个): \033[0mpython3 frpsb.py cat\n")
        f.write("\033[36m│ \033[32m升级脚本: \033[0mpython3 frpsb.py update\n")
        f.write("\033[36m│ \033[32m卸载脚本: \033[0mpython3 frpsb.py del\n")
        f.write("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m\n")
    ...
    with open(str(LIST_FILE) + ".txt", 'w') as f:
        ...
        f.write("---------------------------------------------------------\n")
        f.write("单行节点文件路径: ~/.frpsb/allnodes.txt\n")
        f.write("使用方法:\n")
        f.write("查看节点信息: python3 frpsb.py status\n")
        f.write("查看所有节点(一行一个): python3 frpsb.py cat\n")
        f.write("升级脚本: python3 frpsb.py update\n")
        f.write("卸载脚本: python3 frpsb.py del\n")
        f.write("---------------------------------------------------------\n")
    ...
    with open(str(readme_file), 'w') as f:
        ...
        f.write("## 单行格式节点文件\n\n")
        f.write("如果您需要每行一个节点的格式，可以查看文件: `~/.frpsb/allnodes.txt`\n\n")
        f.write("```bash\ncat ~/.frpsb/allnodes.txt\n```\n\n")
    ...
    print("\033[36m│ \033[32m使用 \033[33mpython3 frpsb.py status\033[32m 查看节点信息\033[0m")
    print("\033[36m│ \033[32m使用 \033[33mpython3 frpsb.py del\033[32m 删除节点\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
...
def install():
    ...
    if not os.path.exists(frp_path):
        ...
        try:
            ...
            # 清理解压后的文件
            if os.path.exists(str(INSTALL_DIR / frpname)):
                shutil.rmtree(str(INSTALL_DIR / frpname))
...
def uninstall():
    ...
    try:
        ...
        print("正在停止frpc服务...")
        if os.path.exists(str(FRPC_PID_FILE)):
            with open(str(FRPC_PID_FILE), 'r') as f:
                pid = f.read().strip()
                if pid:
                    os.system("kill {} 2>/dev/null || true".format(pid))
        ...
        frpc_running = subprocess.run("pgrep -f 'frpc'", shell=True, stdout=subprocess.PIPE).returncode == 0
        ...
        if frpc_running:
            print("尝试强制终止frpc进程...")
            os.system("pkill -9 -f 'frpc' 2>/dev/null || true")
    except Exception as e:
...
def upgrade():
    try:
        script_content = http_get("https://raw.githubusercontent.com/zhiact/argosb/main/frpsb.py")
        if script_content:
...
def check_status():
    try:
        ...
        if sing_box_running and frpc_running and os.path.exists(str(LIST_FILE)):
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            print("\033[36m│                \033[33m✨ frpSB 运行状态 ✨                    \033[36m│\033[0m")
            print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
            ...
            return True
        elif not sing_box_running and not frpc_running:
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            print("\033[36m│                \033[33m✨ frpSB 运行状态 ✨                    \033[36m│\033[0m")
            print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
            print("\033[36m│ \033[31mfrpSB脚本未运行\033[0m")
            print("\033[36m│ \033[32m运行 \033[33mpython3 frpsb.py\033[32m 开始安装\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            return False
        else:
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            print("\033[36m│                \033[33m✨ frpSB 运行状态 ✨                    \033[36m│\033[0m")
            print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
            print("\033[36m│ \033[31mfrpSB脚本状态异常\033[0m")
            print("\033[36m│ \033[32m建议卸载后重新安装: \033[33mpython3 frpsb.py del\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            return False
    except Exception as e:
        print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
...
# 创建frpc配置
def create_frpc_config(port_vm_ws):
    write_debug_log(f"创建frpc配置，端口: {port_vm_ws}")
...
''' % (FRPS_ADDR,FRPS_PORT,FRPS_TOKEN,port_vm_ws,FRPS_REMOTE_PORT)
    
    # 写入配置文件
    frpc_config_file = INSTALL_DIR / "frpc.toml"
    with open(str(frpc_config_file), 'w') as f:
        f.write(config_str)
    
    write_debug_log(f"frpc配置已写入文件: {frpc_config_file}")
...
''')
    ...
    with open(str(cf_start_script), 'w') as f:
        f.write(f'''#!/bin/bash
cd {INSTALL_DIR}
./frpc -c frpc.toml > frpc.log 2>&1 & echo $! > frpcpid.log
''')
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if check_status():
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            print("\033[36m│                \033[33m✨ frpSB 已在运行 ✨                    \033[36m│\033[0m")
            print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
            print("\033[36m│ \033[32mfrpSB脚本已在运行，如需重新安装请先卸载\033[0m")
            print("\033[36m│ \033[32m卸载命令: \033[33mpython3 frpsb.py del\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            sys.exit(0)
        else:
            print("\033[36m╭───────────────────────────────────────────────────────────────╮\033[0m")
            print("\033[36m│               \033[33m✨ 开始安装 frpSB ✨                    \033[36m│\033[0m")
            print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
            install()
